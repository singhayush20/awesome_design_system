# Implementation Plan: Color Architecture Refactor

## Overview

Refactor the design system's color architecture from a three-layer system (`BaseColors` → `ColorTokens` + `DsColors`) to a two-layer system (`BaseColors` → `ColorTokens`) where `ColorTokens` becomes the single semantic color authority containing all Material 3 roles PLUS all component-level semantics currently duplicated in `DsColors`. Remove `DsColors` entirely.

## Architecture Decisions

- **Keep `BaseColors`** as the raw palette foundation (~80 static colors)
- **Extend `ColorTokens`** to include ALL semantic roles from `DsColors` (status, text, background, border, button states, text field states, icon, utility)
- **Remove `DsColors`** entirely - its ~140 colors are merged into `ColorTokens`
- **Update `DesignTokens`** to use the enhanced `ColorTokens` for light/dark theme generation
- **Migrate all consuming packages** (atoms, molecules, organisms, widgetbook) to use `ColorTokens` via `Theme.of(context).extension<ColorTokens>()`
- **Components use semantic roles** from `ColorTokens`, never raw `BaseColors` or `DsColors`
...
- [x] analyzer passes on tokens package
...
|------|--------|------------|
| Missing a semantic role during migration | Components use wrong colors | Comprehensive role mapping from DsColors → ColorTokens before starting |
| Light/dark generation bugs | Wrong colors in one theme | Test every role in widgetbook for both themes |
| Breaking consuming apps | Migration friction | ColorTokens API mirrors DsColors names where possible |
| Tonal palette generation gaps | Some roles lack dark variants | Explicitly define dark values for all new roles in factories |

## Open Questions

- Should any DsColors roles be renamed for consistency with M3 naming?
- Do we need a migration guide for external consumers?
- Should base_colors.dart deprecated status constants be removed?

## File Impact Summary

**Modified:**
- `packages/tokens/lib/src/color_tokens.dart` - Major extension (+100 roles)
- `packages/tokens/lib/src/design_tokens.dart` - Update theme generation
- `packages/tokens/lib/tokens.dart` - Barrel exports
- All atom/molecule/organism component files - Migration to ColorTokens
- `widgetbook/lib/stories/*` - Story updates

**Deleted:**
- `packages/tokens/lib/src/ds_colors.dart`

**New:**
- None (roles added to existing ColorTokens)